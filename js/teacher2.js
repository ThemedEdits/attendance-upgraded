...
function buildRosterForSelection() {
  ...
  roster = students.map((s) => ({
    ...
    SeatNumber: s.SeatNumber || s.StudentID,
    Name: s.Name,
...
function getVisibleRoster() {
  ...
  return roster.filter((r) =>
    ...
    String(r.SeatNumber).toLowerCase().includes(q) ||
      ...
      <table class="ledger">
        <thead>
          <tr>
            ...
            <th style="width:132px;">Seat number</th>
...
function renderRosterRows() {
  ...
  if (currentIds !== nextIds) {
    if (!visible.length) {
      tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; color:var(--text-muted); padding:2rem 1rem;">No students match "${escapeHtml(searchQuery)}".</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = visible.map((r) => {
      ...
      return `
        <tr data-row-index="${index}" style="cursor:pointer;">
          ...
          <td style="width:132px;"><span class="student-id-code">${escapeHtml(r.SeatNumber)}</span></td>
  ...
  rosterWrap.innerHTML = `
    ...
    <div class="table-responsive-container">
      <table class="ledger">
        ...
        <tbody>${rows || `<tr><td colspan="4" style="text-align:center; color:var(--text-muted); padding:2rem 1rem;">No students match "${escapeHtml(searchQuery)}".</td></tr>`}</tbody>
      </table>
    </div>
  `;
